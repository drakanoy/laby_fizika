...
The pump pulse contains a large number of photons, so the corresponding mode is
treated as a classical field with the Rabi frequency $\Omega_R(t)$, whose envelope
defines the pulse area $\theta = 2 \int \Omega_R(t) \, dt$.
The pulse is linearly polarized and thus drives both spin branches with equal
amplitude.
...
split by the Zeeman energy $2 \varepsilon_Z$, where
$\varepsilon_Z = \mu_B g_e B_x$, and their populations follow the Boltzmann
distribution at the lattice temperature $T$.
The initial density matrix therefore reads

\begin{gather}
    \hat{\rho}_0 =
    \frac{\e^{\varepsilon_Z / k_B T}}{2 \cosh \left( \varepsilon_Z / k_B T \right)}
    \left( \frac{\hat{a}^{\dag}_{\uparrow} + \hat{a}^{\dag}_{\downarrow}}{\sqrt{2}} \right)
    \left| 0 \right> \left< 0 \right|
    \left( \frac{\hat{a}_{\uparrow} + \hat{a}_{\downarrow}}{\sqrt{2}} \right)
    \label{eq:rho0}
    \\ \nonumber
    + \frac{\e^{-\varepsilon_Z / k_B T}}{2 \cosh \left( \varepsilon_Z / k_B T \right)}
    \left( \frac{\hat{a}^{\dag}_{\uparrow} - \hat{a}^{\dag}_{\downarrow}}{\sqrt{2}} \right)
...
$\tanh \left( \varepsilon_Z / k_B T \right) / 2$.
The single parameter $\varepsilon_Z / k_B T$ thus controls the degree of the initial
spin coherence.
For $\varepsilon_Z \gg k_B T$ the electron is fully polarized along the field and the
system starts from a pure state, whereas for $\varepsilon_Z \ll k_B T$ the initial
