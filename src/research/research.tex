...
split by the Zeeman energy $2 \mu_B g_e B_x$, and their populations follow the
Boltzmann distribution at the lattice temperature $T$.
The initial density matrix in the basis
$\{ \hat{a}^{\dag}_{\uparrow} \left| 0 \right> ,
\hat{a}^{\dag}_{\downarrow} \left| 0 \right> \}$ therefore reads

\begin{gather}
    \hat{\rho}_0 = \frac{1}{2}
    \begin{pmatrix}
        1 & \tanh \left( \mu_B g_e B_x / k_B T \right) \\
        \tanh \left( \mu_B g_e B_x / k_B T \right) & 1
    \end{pmatrix}
    \label{eq:rho0}
\end{gather}

The single parameter $\mu_B g_e B_x / k_B T$ controls the degree of the initial spin
coherence.
For $\mu_B g_e B_x \gg k_B T$ the electron is fully polarized along the field and the
system starts from a pure state, whereas for $\mu_B g_e B_x \ll k_B T$ the two spin
projections are populated equally and the initial state is a fully incoherent
mixture.
