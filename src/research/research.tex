...
pulse by a trion that has survived until $t = \tau$ and then recombines with a
...
It is therefore convenient to introduce, along with the solution $\hat{\rho}(t)$ of
Eq.~(\ref{eq:lindblad}) with the initial condition (\ref{eq:rho0}), the conditional
density matrix $\hat{\rho}^{(\lambda)}(t | t_1)$.
The latter is the solution of the same equation propagated from $t_1$ to $t$ with the
initial condition $\hat{b}_{\lambda} \hat{\rho}(t_1) \hat{b}^{\dag}_{\lambda}$, that
is, the state of the system given that a photon of circular polarization $\lambda$
has escaped from the mode at the instant $t_1$.
