...
density matrix $\hat{\rho}^{(\lambda \lambda')}(t | t_1)$, describing the system
given that a photon has escaped from the mode at the instant $t_1$ and carrying the
...
\begin{gather}
    ...
    \\ \nonumber
    \hat{\rho}^{(\lambda \lambda')}(t | t_1) = \e^{\hat{\mathcal{L}} (t - t_1)}
...
Here the operators $\hat{b}_{\lambda}$ and $\hat{b}^{\dag}_{\lambda'}$ remove one
photon from the mode and hand it over to the detector, after which the remaining part
of the pulse drives the system further.
%The two polarization indices are kept separate, so that
%$\hat{\rho}^{(\lambda \lambda')}$ is a density matrix only for $\lambda = \lambda'$,
%while its off-diagonal components carry the polarization coherence of the escaped
%photon.
...
\begin{gather}
    ...
    \left< 0 \right| \hat{a}_{\sigma}
    \hat{\rho}^{(\lambda \lambda')}(\tau | t_1)
...
Before turning to the numerical solution it is instructive to look at the limiting
form of $\hat{\rho}_2$. % that can be obtained in closed form.
To first order in the small parameter $g \tau$, neglecting the phonon induced
dephasing but keeping the spin relaxation of the resident electron after pulse pump, the
two-photon density matrix generated by a $2\pi$ pulse reads, in the basis
...
Only the $\left| + + \right> \pm \left| - - \right>$  states are populated, % as dictated by the selection rules,
and the entire dependence on the magnetic field and on the temperature enters through the coherence $\eta^2$.
...
emission (\ref{eq:spin-projection}).
The factor $\Omega_R^2 / ( \Omega_R^2 + \Delta^2 )$ shows how the
detuning influences the two-photon yield.

Equation~(\ref{eq:rho2-analytic}) is a mixture of the two Bell states with thermal
weights,
...
\left| - - \right> ) / \sqrt{2}$.
In the limit of a strong spin polarization, $\mu_B g_e B_x \gg T$, one has
$\eta \rightarrow 1$ and only $\left| \Phi^+ \right>$ survives, whereas for
$\mu_B g_e B_x \ll T$ the two Bell states enter with equal weights and the pair is no
longer entangled.
%The exact results presented below lie somewhat lower than these estimates, since the
%phonon dephasing and the finite $\Gamma \tau$ are then taken into account.
