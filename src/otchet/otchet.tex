...
%
%Источники одиночных фотонов являются ключевыми компонентами квантовых фотонных процессоров, лежащих в основе таких прикладных направлений,
%как линейно-оптические квантовые вычисления и квантовая криптография.
%Для успешной реализации этих технологий необходимы источники,
%максимально приближенные к «идеальному»: быть детерминированными (они должны обеспечивать строго один фотон по запросу пользователя),
%иметь высокую вероятность генерации фотона (близкую к 100 \%),
%минимальную вероятность генерации многофотонных событий (двух и более фотонов),
%производить фотоны с высокой степенью неразличимости, а также обладать высокой скоростью генерации.
%
%Подобные структуры изучаются методом кросс-поляризации.
%Мы предлагаем подход который позваляет получать запуанные состояния фотонов исразу из одного источника,
%при этом не используя метод крос поляризации, использую полуризацию в качестве запутаности.
%
%Мы представляем результаты о наблюдении группы фотонов в состояниях,
%содержащих $\sim$ 3 фотона в нескольких поляризациях, представляющих собой когерентную суперпозицию
...
%Возникновение явления обусловлено тем,
%используя отстройку в накачке от энергии экситона и фильтруя лазерные состояния по частоте, а не поляризации.
%Подобный пучок фотонов является состоянием высшего порядка в
...
\subsection{Новый подход к старой задче}
В рамках предыдущих работ и точного вычисления было показано, что у двухуровневой системы, описываемой Гамельтонианом:
\begin{equation}
    \hat{H} = E_0 \ \hat{a}^{\dag}_{+} \hat{a}_{+} + \omega_0 \ \hat{b}^{\dag}_{-} \hat{b}_{-} +  g \left( \hat{a}^{\dag}_{+} \hat{b}_{-} + \hat{a}_{+} \hat{b}^{\dag}_{-} \right)
\end{equation}
...
\begin{gather}
    g^{(2)}(0) = \frac{ \left< \psi_{out}^V \left| \ \hat{n}^2 - \hat{n} \ \right| \psi_{out}^V \right>}{\left< \psi_{out}^V \left| \ \hat{n}^2 \ \right| \psi_{out}^V \right> ^2 } = \frac{2\left| \delta \right|^2 \cos^2 \left( \frac{\theta}{2} \right) }{\left( \sin^2 \left( \frac{\theta}{2}\right) + \left| \delta \right|^2 \right)^2}
\end{gather}
...
\begin{gather}
    g^{(2)}(0) = \frac{ \left< \psi_{out}^V \left| \ \hat{n}^2 - \hat{n} \ \right| \psi_{out}^V \right>}{\left< \psi_{out}^V \left| \ \hat{n} \ \right| \psi_{out}^V \right> ^2 } = \frac{2 \left( 1+\delta_N^2 \right) \gamma^2 \cos^2 \frac{\theta}{2} }{\left( \gamma^2 \left( 1 + \delta_N^2 \right) + \sin^2 \frac{\theta}{2} - \delta_N \gamma \sin \theta \right)^2}
\end{gather}
